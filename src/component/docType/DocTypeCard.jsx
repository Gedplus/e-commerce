...
import { addWishlist, editDocumentDE, editDocumentDI, editDocumentDU, editDocumentE, editDocumentI, editDocumentP, editDocumentU, getDocType, getDocUni, useGetDocumentQuery, useGetUtilisateursQuery } from '../../state/api'
import { Button, CardActions, CircularProgress } from "@mui/material";
import Pagination from "../shop/pagination";

const DocTypeCard = ({ shopItems, addToCart ,user }) => {
  
  const userss = useGetUtilisateursQuery();
  const { id } = useParams();
  console.log("idsahar",id)
...
( <>   {currentRecords === undefined  || users === undefined  ? (<><CircularProgress className="addProgress1" /></>) : (<>{currentRecords.map((shopItems, index) => { 
        return (
        <> 
